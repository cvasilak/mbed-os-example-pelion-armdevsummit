...
void capsense_main(void)
{
    ...
    {
        printf("CapSense initialization failed. Status code: %d\r\n", status);
    ...
    thread.start(callback(&capqueue, &EventQueue::dispatch_forever));
    capqueue.call_every(CAPSENSE_SCAN_PERIOD_MS, RunCapSenseScan);
